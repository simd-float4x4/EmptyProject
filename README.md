# SwiftUI MVC Template

SwiftUIとMVCアーキテクチャを組み合わせたiOSアプリのテンプレートリポジトリです。
...
| **Controller** | ViewとModelの仲介。ビジネスロジックの処理。ObservableObjectを実装 |
...
│       └── DetailRouter.swift
Scripts/
└── generate_module_mvc.sh    # モジュール生成スクリプト
```

## モジュール生成

新しいMVCモジュールを作成するには、以下のコマンドを実行します：

```bash
./Scripts/generate_module_mvc.sh <ModuleName>
```
...
```bash
# Settingsモジュールを作成
./Scripts/generate_module_mvc.sh Settings

# UserProfileモジュールを作成
./Scripts/generate_module_mvc.sh UserProfile
```
...
- `<ModuleName>Controller.swift` - コントローラー
...
- SwiftUI Viewを使用
- Controllerを`@ObservedObject`で監視
- UI表示のみを担当
